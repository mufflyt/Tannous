...
isochrones$range <- as.factor(isochrones$range)
#```

#```{r map_isochrones, include=TRUE}
color_palette <- viridis::magma(length(unique(isochrones$range)))

isochrone_map <- tyler::create_base_map("") %>%
  addProviderTiles("CartoDB.Positron", group = "Greyscale") %>%
  addPolygons(
    data = isochrones,
    fillColor = ~color_palette[match(range, unique(isochrones$range))],
  ...
  leaflet::addCircleMarkers(
    ...
    fillOpacity = 0.1,
    color = "#1f77b4",
    popup = ~paste0("<strong>Name:</strong> ", address, "<br />",
  ...
  addLegend(
    ...
    labels = unique(isochrones$range),  # Corrected to use drive_time
    title = "Drive Time (minutes)",
    opacity = 0.3
  ) %>%
  addLayersControl(options = layersControlOptions(collapsed = FALSE),
                   overlayGroups = c("Isochrones", "Gynecologic Oncologists")); isochrone_map
